...
type navigatorHotelView struct {
	searchCode  string
	searchQuery string
	results     core.NavigatorSearchResults
}

func NewNavigatorHotelView(searchCode string, searchQuery string, results core.NavigatorSearchResults) *navigatorHotelView {
	return &navigatorHotelView{
...
func (c *navigatorHotelView) Compose(compose core.OutgoingPacket) {
	...
	for _, r := range c.results.Rooms {
		room := r.(room_info.GetActiveRoomsRow)
